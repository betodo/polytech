...
// 			console.log(i, item);
			...
			if (e1.text != '예약가능') {
				e1.classList.add("booked"); //클래스를 붙여줌
				 $(".booked").removeAttr("href"); // 클래스의 href를 없애줌
				 e1.style.color = "red"; //e1의 스타일을 바꿔줌 
			}
			if(e2.text !== '예약가능') {
				e2.classList.add("booked");
				$(".booked").removeAttr("href");
				 e2.style.color = "red";
			}
			if(e3.text !== '예약가능') {
	...
	<table id="basicTB" width="100%">
		...
		<tbody>
			...
			<tr class="reservation_by_day">
				...
				<td><a class="room1"
					href="?contentPage=Reserve/Reserve_insert.jsp?day=<%=rset.getString(1)%>&room=1"><%=rset.getString(3) %></a></td>
				<td><a class="room2"
					href="?contentPage=Reserve/Reserve_insert.jsp?day=<%=rset.getString(1)%>&room=2"><%=rset.getString(4) %></a></td>
				<td><a class="room3"
					href="?contentPage=Reserve/Reserve_insert.jsp?day=<%=rset.getString(1)%>&room=3"><%=rset.getString(5) %></a></td>
			</tr>
			<%
				}
				rset.close();
